...
void traverse_slab_pages(void)
{
	...
	unsigned long total_pages = get_num_physpages();
	unsigned long slab_pages = 0;

	page = pfn_to_page(0);
	pr_info("page0: %016lx %016lx\n", (unsigned long) page, page->flags);
	page = pfn_to_page(1);
	pr_info("page0: %016lx %016lx\n", (unsigned long) page, page->flags);
	for (i = 0;i < total_pages;i++) {
		page = pfn_to_page(i);
		// if (page->slab_cache == 0) {
		// 	continue;
		// }
		// if (page->slab_cache <= 0xffff888000000000 || page->slab_cache >= 0xffffc88000000000) {
		// 	// pr_info("not slab_cache? %016lx\n", page->slab_cache);
		// } else 
		if (PageSlab(page)) {
			if (page->slab_cache <= 0xffff888000000000 || page->slab_cache >= 0xffffc88000000000) {
				pr_info("not slab_cache? %016lx\n", page->slab_cache);
			...
			++slab_pages;
			oo = page->slab_cache->oo;

			pr_info("slab pages:%d: %016lx %s %u, pages:%u\n", i, page->slab_cache, page->slab_cache->name, page->slab_cache->object_size, 1 << (oo_order(oo)));
			if (slab_pages % 100 == 0) {
				pr_info("=========%lx slab_pages==========\n", slab_pages);
			}
...
EXPORT_SYMBOL(traverse_slab_pages);
