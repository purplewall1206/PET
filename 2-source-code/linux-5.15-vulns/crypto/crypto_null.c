...
static int null_skcipher_crypt(struct skcipher_request *req)
{
	...
	// pr_info("null_skcipher_crypt: dst:%016lx, src:%016lx len:0x%x\n", (unsigned long)walk.dst.virt.addr, (unsigned long)walk.src.virt.addr, walk.nbytes);

	while (walk.nbytes) {
		if (walk.src.virt.addr != walk.dst.virt.addr) {
			memcpy(walk.dst.virt.addr, walk.src.virt.addr,
			       walk.nbytes);
			// pr_info("\tnull_skcipher_crypt: dst:%016lx, len:0x%x\n", (unsigned long)walk.dst.virt.addr, walk.nbytes);
		}
