			...
			union tpacket_req_u *req_u)
{
	...
	// pr_info("init_prb_bdqc:\n\tp1->blk_sizeof_priv:0x%x\n\treq_u->req3.tp_sizeof_priv:0x%x\n", 
	// 		p1->blk_sizeof_priv, req_u->req3.tp_sizeof_priv);
	...
	struct tpacket_block_desc *pbd1)
{
	...
	// pkc1->nxt_offset指向缓冲区ring_buffer中block当前可接收数据的起始地址，
	// 由于pkc1->blk_sizeof_priv可控，因此可以控制pkc1->nxt_offset在接收数据包时造成堆越界。
	// pr_info("prb_open_block: pkc1->nxt_offset OOB start:%016lx, nxt_offset:%016lx\n", 
	// 		(unsigned long)pkc1->pkblk_start, (unsigned long)pkc1->nxt_offset );
	...
	// pr_info("prb_open_block: pkc1->pkblk_end OOB start:%016lx, end:%016lx\n", 
	// 		(unsigned long)pkc1->pkblk_start, (unsigned long)pkc1->pkblk_end );
		       ...
		       struct packet_type *pt, struct net_device *orig_dev)
{
	...
	// remove fix patch for cve-2020-14386
	// unsigned int netoff;
	...
	if (sk->sk_type == SOCK_DGRAM) {
		...
		netoff = TPACKET_ALIGN(po->tp_hdrlen +
				       (maclen < 16 ? 16 : maclen)) +
				       po->tp_reserve;
		// po->tp_reserve is cutoff here by netoff
		if (po->has_vnet_hdr) {
			netoff += sizeof(struct virtio_net_hdr);
			do_vnet = true;
		}
		macoff = netoff - maclen; // *******underflow here***********
	}
	...
	// check h.raw + macoff -sizeof(struct virtio_net_hdr)  *******underflow point*******
	// pr_info("cve-2020-14386: h.raw:%016lx, underflow:%016lx\n", (unsigned long)h.raw, 
	// 										(unsigned long )(h.raw + macoff -sizeof(struct virtio_net_hdr)));
	if (do_vnet &&
		  ...
		  unsigned int optlen)
{
	...
	case PACKET_RESERVE:
	{
		...
		// if (val > INT_MAX)
		// 	return -EINVAL;
		// pr_info("packet_setsockopt  PACKET_RESERVE  val:%u\n", val);
		lock_sock(sk);
		if (po->rx_ring.pg_vec || po->tx_ring.pg_vec) {
			ret = -EBUSY;
		} else {
		...
		int closing, int tx_ring)
{
	...
	if (req->tp_block_nr) {
		...
		// 	goto out;
		// pr_info("packet_set_ring:int oob\n\treq->tp_block_size:0x%x\n\tBLK_PLUS_PRIV(req_u->req3.tp_sizeof_priv):0x%x\n\tsub:0x%x\n",
		...
		// 	goto out;
		// pr_info("packet_set_ring: invalid cmp rb->frames_per_block:0x%x, max:0x%x, cmp:%d\n", 
		// 		rb->frames_per_block, (UINT_MAX / req->tp_block_nr), 
		// 		(rb->frames_per_block > UINT_MAX / req->tp_block_nr));
		...
		// allocate pg_vec files
		// pr_info("packet_set_ring:req->tp_block_size:%x, order:%d\n", req->tp_block_size, order);
		pg_vec = alloc_pg_vec(req, order);
		if (unlikely(!pg_vec))
			goto out;

		// pr_info("packet_set_ring:switch (po->tp_version), tx_ring:%d\n", tx_ring);
		switch (po->tp_version) {
		case TPACKET_V3:
