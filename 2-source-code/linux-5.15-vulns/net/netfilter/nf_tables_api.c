			...
			const u32 *data, u64 timeout, u64 expiration, gfp_t gfp)
{
	...
	pr_info("nft_set_elem_init  kzmalloc %u %u %u %016lx\n",set->ops->elemsize,  tmpl->len,set->ops->elemsize + tmpl->len, elem);
	if (elem == NULL)
		return NULL;
	...
	if (nft_set_ext_exists(ext, NFT_SET_EXT_DATA)) {
		pr_info("[1] nft_set_elem_init  %016lx %016lx %u\n", nft_set_ext_data(ext), data, set->dlen);
		memcpy(nft_set_ext_data(ext), data, set->dlen);  // out-of-bound !!!! here 
	}
			    ...
			    const struct nlattr *attr, u32 nlmsg_flags)
{
	...
	elem.priv = nft_set_elem_init(set, &tmpl, elem.key.val.data,
				      ...
				      timeout, expiration, GFP_KERNEL);  // OOB here
	if (elem.priv == NULL)
		goto err_parse_data;
	...
	if (obj) {
		*nft_set_ext_obj(ext) = obj;
		// pr_info("CVE-2022-2586: nft_set_ext_obj(ext):%016lx, ext:%016lx, obj: %016lx\n", nft_set_ext_obj(ext), ext, *nft_set_ext_obj(ext));
		obj->use++;
	}
...
static void nft_obj_destroy(const struct nft_ctx *ctx, struct nft_object *obj)
{
	...
	kfree(obj->key.name);
	// pr_info("CVE-2022-2586:nft_obj_destroy: obj->key.name:%016lx\n", obj->key.name);
	kfree(obj->udata);
	// pr_info("CVE-2022-2586:nft_obj_destroy: obj->udata:%016lx\n", obj->udata);
	kfree(obj);
	// pr_info("CVE-2022-2586:nft_obj_destroy: obj:%016lx\n", obj);
}
...
static unsigned int nft_parse_register(const struct nlattr *attr)
{
	...
	case NFT_REG_VERDICT...NFT_REG_4:
		pr_info("nft_parse_register REG %x  %x %x\n", reg , NFT_REG_SIZE / NFT_REG32_SIZE,reg * NFT_REG_SIZE / NFT_REG32_SIZE);
		return reg * NFT_REG_SIZE / NFT_REG32_SIZE;
	default:
		pr_info("nft_parse_register DEFAULT %x  %x %x\n", reg , NFT_REG_SIZE / NFT_REG32_SIZE,reg * NFT_REG_SIZE / NFT_REG32_SIZE);
...
int nft_parse_register_load(const struct nlattr *attr, u8 *sreg, u32 len)
{
	...
	// pr_info("nft_parse_register_load reg: %x\n", reg);
	err = nft_validate_register_load(reg, len);
	// pr_info("nft_parse_register_load reg err: %d\n", err);
	if (err < 0)
		return err;
			     ...
			     enum nft_data_types type, unsigned int len)
{
	...
	reg = nft_parse_register(attr);
	// pr_info("nft_parse_register_store reg: %x\n", reg);
	err = nft_validate_register_store(ctx, reg, data, type, len);
	// pr_info("nft_parse_register_store reg err: %d\n", err);
	if (err < 0)
		return err;
